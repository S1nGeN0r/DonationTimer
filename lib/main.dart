...
class LogManager {
  ...
  static Future<void> init() async {
    ...
    log(Level.INFO,
        'Логирование инициализировано. Путь к файлу логов: $_logFilePath');
  }
...
class _MainScreenState extends State<MainScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          LogManager.log(
              Level.INFO, 'External IP (for WebSocket): $_localIpAddress');
        });
        return;
      } else {
        LogManager.log(Level.WARNING,
            'Failed to fetch external IP, falling back to local');
      }
    } catch (e) {
  ...
  void _initTrulaListener() {
    if (_trulaToken.isEmpty) {
      LogManager.log(Level.INFO, 'Trula: токен отсутствует');
      return;
    }

    _trulaListener = TrulaDonationListener(
      token: _trulaToken,
      onDonation: (username, amount) {
        LogManager.log(Level.INFO, 'Trula: Донат от $username на $amount ₽');
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title:
                Text(context.read<LocalizationProvider>().translate('remote')),
            content: Column(
            ...
            actions: [
              TextButton(
                child: Text(
                    context.read<LocalizationProvider>().translate('close')),
                onPressed: () {
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          title: Column(
            children: <Widget>[
              Text(
                  '★ ${context.read<LocalizationProvider>().translate('info')} ★')
            ],
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              RichText(
                text: TextSpan(
                  text:
                      '${context.read<LocalizationProvider>().translate('author')}: ',
              ...
              RichText(
                text: TextSpan(
                  text:
                      '${context.read<LocalizationProvider>().translate('version')}: ',
                  style: theme.textTheme.bodyLarge,
                  children: <TextSpan>[
                    TextSpan(
                      text: version,
                      ...
                      recognizer: TapGestureRecognizer()
                        ..onTap = () async {
                          final Uri url = Uri.parse(
                              'https://github.com/MjKey/DonatonTimer/releases');
                          if (await canLaunchUrl(url)) {
                            await launchUrl(url);
                          } else {
                            throw 'Не удалось открыть $url';
          ...
          actions: <Widget>[
            Row(
              ...
              children: <Widget>[
                TextButton(
                  child: Text(
                      context.read<LocalizationProvider>().translate('okpon')),
                  onPressed: () {
                    Navigator.of(context).pop();
                  },
                ),
  ...
  void _saveTimerDuration() async {
    LogManager.log(
        Level.INFO, 'Сохранение длительности таймера: $_timerDuration');
    SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _initSocketService() async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('donation_alerts_token');
    String socketUrl = await _extractSocketUrl(_newWidgetUrl) ??
        prefs.getString('socket_url') ??
        _socketUrl;
    if (token != null) {
      _tokenPreview =
          '${token.substring(0, 3)}...${token.substring(token.length - 3)}';
  ...
  void _handleDonation(dynamic data) {
    ...
    if (_processedDonations.contains(donationData['id'])) {
      LogManager.log(
          Level.WARNING, 'Донат с ID ${donationData['id']} уже обработан');
      return;
    }
    ...
    if (alert_type == 1 && show == 0 && donationData['currency'] == 'RUB') {
      ...
      LogManager.log(Level.INFO,
          "DONATE INFO {OWNER}: $curr | $show | $bsys | $bsyt | $username | $amountMain | $did");
      ...
      LogManager.log(Level.INFO,
          'Это был не донат. Критерии: currency = $curr (RUB) | is_shown = $show (0) | billing_system = $bsys (!TWITCH) | billing_system_type = $bsyt (!REWARDS)');
      return;
    }
  }
  ...
  void _loadSoundFiles() {
    ...
    if (!_soundDirectory.existsSync()) {
      LogManager.log(Level.WARNING,
          'Папка sound не найдена, создаём её: ${_soundDirectory.path}');
      _soundDirectory.createSync(recursive: true);
      LogManager.log(
          Level.INFO, 'Папка sound создана: ${_soundDirectory.path}');
    } else {
      LogManager.log(
          Level.INFO, 'Папка sound найдена: ${_soundDirectory.path}');
    }

    _soundFiles = _soundDirectory
        .listSync()
    ...
    if (_soundFiles.isNotEmpty) {
      ...
      LogManager.log(Level.INFO,
          'Выбранный файл для воспроизведения: ${_currentSoundFile?.path}');
    } else {
  ...
  void _refreshSoundFolder() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content:
          Text(context.read<LocalizationProvider>().translate('sound_updated')),
    ));
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      showDialog(
        ...
        builder: (BuildContext context) {
          ...
          return AlertDialog(
            title: Text(
                context.read<LocalizationProvider>().translate('start_time')),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(
            ...
            actions: [
              TextButton(
                child: Text(
                    context.read<LocalizationProvider>().translate('setup')),
                onPressed: () {
                  ...
                  _saveSettings();
                  LogManager.log(Level.INFO,
                      'Установлено начальное время: $_timerDuration');
                  Navigator.of(context).pop();
                },
              ),
            ],
  ...
  Future<String?> _extractSocketUrl(String widgetUrl) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final match =
            RegExp(r"wss?:\/\/socket\d+\.donationalerts\.[a-zA-Z.]+:\d+")
                .firstMatch(body);
        if (match != null) {
                    ...
                    Text(
                      _tokenPreview.isNotEmpty
                          ...
                          : context
                              ...
                              .translate('not_token'),
                      style: TextStyle(
                        color: _tokenPreview.isNotEmpty
                            ? Colors.green
                            : Colors.red,
                      ),
                    ),
                  ...
                  onPressed: () async {
                    ...
                    if (newWidgetUrl.isNotEmpty) {
                      ...
                      if (token != null) {
                        ...
                        String? socketUrl =
                            await _extractSocketUrl(newWidgetUrl);
                        if (socketUrl != null) {
  ...
  void _saveStatistics() async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> recentDonationsJson = _recentDonations
        .map((record) => json.encode(
            {'username': record.username, 'minutesAdded': record.minutesAdded}))
        .toList();
    await prefs.setStringList('recent_donations', recentDonationsJson);

    List<String> topDonatorsJson = _topDonators.entries
        .map((e) => json.encode({'username': e.key, 'minutesAdded': e.value}))
        .toList();
  ...
  void _resetSettings() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            context.read<LocalizationProvider>().translate('settings_clear'))));
  }
...
<body>
  ...
  void _copyLinkToClipboard() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content:
            Text(context.read<LocalizationProvider>().translate('OBScopy'))));
  }
  ...
  void _playSound() async {
    if (_isSoundEnabled && _currentSoundFile != null) {
      try {
        LogManager.log(
            Level.INFO, 'Проигрывается звук: ${_currentSoundFile?.path}');
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            tooltip:
                context.read<LocalizationProvider>().translate('m_settings'),
            onSelected: (value) {
              switch (value) {
                case 'settings':
                  _showSettingsDialog();
        ...
        children: [
          Expanded(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(_formatDuration(_timerDuration),
                    style: GoogleFonts.lato(
                      textStyle: const TextStyle(
                ...
                Row(
                  ...
                  children: [
                    ElevatedButton(
                        onPressed: () => _changeTimer(-60),
                        child: Text(context
                            .read<LocalizationProvider>()
                            .translate('-min'))),
                    const SizedBox(width: 10),
                    ElevatedButton(
                        onPressed: () => _changeTimer(-600),
                        child: Text(context
                            .read<LocalizationProvider>()
                            .translate('-10min'))),
                    const SizedBox(width: 10),
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              trailing: Text(
                                  '${amount.toStringAsFixed(2)}₽ / ${DonationRecord.formattedTime(context, donation.minutesAdded)}'),
                            );
                          },
                        ),
                      ),
